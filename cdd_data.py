...
# CitrusDisease6数据集处理
class CitrusDisease7_preprocess():
    def __init__(self, path=None):
        self.raw_path = os.getcwd() + '/datasets/cdd_raw/'
        self.path = path
        self.label_names = ['blackspot', 'canker', 'greening', 'healthy', 'melanose']
    ...
    def image_rename(self):
        ...
        for i in range(len(folders)):
            folder_path = os.path.join(self.raw_path, self.label_names[i])
            save_path = os.path.join(self.path, self.label_names[i])
            ...
            for j in range(len(image_names)):
                ...
                rename = str(i) + '_' + self.label_names[i] + '_' + str(j) + '.jpg'
    ...
    def remove_non_image_files(self, image_names):
        ...
        for i in range(len(image_names)):
            if image_names[i].split('.')[-1] == 'jpg' or image_names[i].split('.')[-1] == 'png':
                new_image_names.append(image_names[i])
        return new_image_names

    def train_test_split(self, split_rate=0.8):
        train_dic, test_dic = {}, {}
...
if __name__ == "__main__":
    P = CitrusDisease7_preprocess(path=os.getcwd() + '/datasets/cdd/')
    # P.image_rename()
    # P.add_image()
    P.train_test_split()
