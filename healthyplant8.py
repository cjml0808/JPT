...
# HealthyPlant8数据集处理
class HealthyPlant8_preprocess():
    def __init__(self, path=None):
        self.raw_path = os.getcwd() + '/datasets/healthyplant8_raw/'
        self.path = path
        self.label_names = ['apple', 'bellpepper', 'blueberry', 'cherry', 'soybean', 'potato', 'raspberry', 'strawberry']
        # self.label_names = {'0': 'normal', '1': 'anthracnose', '2': 'canker',
        #                     '3': 'huanglong', '4': 'melanose', '5': 'sunscald', '6': 'blackspot'}
        # self.label_names = {'0': '2_canker', '1': '6_blackspot'}
        # self.ori_label_names = {'0': 'citrus-canker', '1': 'black-spot'}
        self.class_num = 8
        self.data_splits = ['train', 'test', 'pretrain', 'pretest']
        self.image_size = (256, 256)

    def image_rename(self):
        label = 0
        for label_name in self.label_names:
            folder_path = os.path.join(self.raw_path, label_name)
            save_path = os.path.join(self.path, label_name)
            ...
            for j in range(len(image_names)):
                ...
                rename = str(label) + '_' + label_name + '_' + str(j) + '.jpg'
    ...
    def add_image(self):
        folders_1 = 'E:/Data/dataset/dataset/train/'
        folders_2 = 'E:/Data/dataset/dataset/test/'

        # for i in range(len(folders_1)):
        #     folder_path = os.path.join(folders_1, self.ori_label_names[str(i)])
        #     save_path = os.path.join(self.path, self.label_names[str(i)].split('_')[1])
        #     if not os.path.exists(save_path):
        #         os.makedirs(save_path)
        #
        #     current_num = len(os.listdir(save_path))
        #     image_names = os.listdir(folder_path)
        #     image_names = self.remove_non_image_files(image_names)
        #
        #     for j in range(len(image_names)):
        #         image = Image.open(os.path.join(folder_path, image_names[j]))
        #         resize_image = image.resize(self.image_size)
        #         rename = self.label_names[str(i)] + '_' + str(j + current_num) + '.jpg'
        #
        #         resize_image.save(os.path.join(save_path, rename))
        #         print(f"Image {rename} is saved successfully!")

        for i in range(len(folders_2)):
            folder_path = os.path.join(folders_2, self.ori_label_names[str(i)])
            save_path = os.path.join(self.path, self.label_names[str(i)].split('_')[1])
            if not os.path.exists(save_path):
                os.makedirs(save_path)
    ...
    def train_test_split(self, split_rate=0.95):
        ...
        for i in range(self.class_num):
            ...
            for j in range(len(image_names)):
                ...
                if int(len(image_names) * 0.8) <= j < int(len(image_names) * 0.9) or \
                        int(len(image_names) * 0.95) <= j < int(len(image_names) * 1.0):
                ...
                elif int(len(image_names) * 0.9) <= j < int(len(image_names) * 0.95):
    ...
    def pretrain_pretest_split(self, split_rate=0.8):
        ...
        random.shuffle(pretrain_data_labels)
        pretrain_data, pretrain_labels, pretrain_filenames = zip(*pretrain_data_labels)
        pretest_data_labels = list(zip(pretest_data, pretest_labels, pretest_filenames))
        random.shuffle(pretest_data_labels)
        pretest_data, pretest_labels, pretest_filenames = zip(*pretest_data_labels)
...
class HealthyPlant8(VisionDataset):
...
class _HealthyPlant5(VisionDataset):
    """`CIFAR10 <https://www.cs.toronto.edu/~kriz/cifar.html>`_ Dataset.
    ...
    base_folder = "healthyplant5"
    train_file = "train_5"
    test_file = "test_5"
    pretrain_file = 'pretrain_5'
...
class _HealthyPlant3(VisionDataset):
    """`CIFAR10 <https://www.cs.toronto.edu/~kriz/cifar.html>`_ Dataset.
    ...
    base_folder = "healthyplant3"
    train_file = "train_3"
    test_file = "test_3"
    pretrain_file = 'pretrain_3'
...
if __name__ == "__main__":
    P = HealthyPlant8_preprocess(path=os.getcwd() + '/datasets/healthyplant8/')
    # P.image_rename()
    # P.add_image()
    # P.train_test_split()
