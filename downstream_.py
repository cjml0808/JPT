...
from citrus_data_7 import CitrusDisease7, CitrusDisease3, CitrusDisease4, CDD
...
def eval(mode='full_finetune'):
    ...
    num_class = 8
    model = models.create('resnet_ibn50a_4wa_prune', num_features=2048, norm=True, dropout=0,
    ...
    PATH = f"E:/Code/densityclustering-master/Second_work/densityclustering-master/examples/logs/" \
           f"healthyplant8_resnet_ibn50a/healthyplant8_4head_512(4wa)_0.5_prune(0.5)_1"
    if checkpoint_exist:
        checkpoint = torch.load(os.path.join(PATH, 'model_10_0.5_prune.pth.tar'), map_location=device)
        state_dict = checkpoint['state_dict']
        model = copy_state_dict(state_dict, model, strip='module.')

    if mode == 'final_finetune':
        logging.basicConfig(filename=os.path.join(PATH, f"2048_final_testing_{checkpoint['epoch']}_normalize_prune.log"),
                            level=logging.DEBUG)
        ...
        if checkpoint['dataset_name'] == 'citrusdisease7':
            k = 5
            train_loader, test_loader = get_citrusdisease7_data_loaders()
        elif checkpoint['dataset_name'] == 'citrusdisease4':
            k = 3
            train_loader, test_loader = get_citrusdisease4_data_loaders()
        elif checkpoint['dataset_name'] == 'citrusdisease3':
            k = 2
            train_loader, test_loader = get_citrusdisease3_data_loaders()
        elif checkpoint['dataset_name'] == 'citrusdisease6':
            k = 5
            train_loader, test_loader = get_citrusdisease6_data_loaders()
        elif checkpoint['dataset_name'] == 'cdd':
            k = 4
            train_loader, test_loader = get_cdd_data_loaders()
        elif checkpoint['dataset_name'] == 'healthyplant8':
            k = 5
            train_loader, test_loader = get_healthyplant8_data_loaders()
        elif checkpoint['dataset_name'] == 'tomatodisease5':
            k = 4
            train_loader, test_loader = get_tomatodisease5_data_loaders()
        print(f"Dataset: {checkpoint['dataset_name']}")
        logging.info(f"Dataset: {checkpoint['dataset_name']}")
        ...
        logging.info(f"Testing with dataset: {checkpoint['dataset_name']}.")
        begin = time.time()
...
def get_healthyplant8_data_loaders(shuffle=False, batch_size=8):
    train_dataset = HealthyPlant8('./examples/data/healthyplant8', pretrain=False, train=True,
                                  transform=transforms.Compose([transforms.ToTensor(), normalizer]))

    train_loader = DataLoader(train_dataset, batch_size=batch_size,
                              num_workers=2, drop_last=False, shuffle=shuffle, persistent_workers=True)

    test_dataset = HealthyPlant8('./examples/data/healthyplant8', pretrain=False, train=False,
                                 transform=transforms.Compose([transforms.ToTensor(), normalizer]))
...
def get_tomatodisease5_data_loaders(shuffle=False, batch_size=8):
    train_dataset = TomatoDisease5('./examples/data/tomatodisease5', pretrain=False, train=True,
                                   transform=transforms.Compose([transforms.ToTensor(), normalizer]))

    train_loader = DataLoader(train_dataset, batch_size=batch_size,
                              num_workers=2, drop_last=False, shuffle=shuffle, persistent_workers=True)

    test_dataset = TomatoDisease5('./examples/data/tomatodisease5', pretrain=False, train=False,
                                  transform=transforms.Compose([transforms.ToTensor(), normalizer]))
